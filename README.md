...
- `composer.lock` をコミットしているリポジトリ向けです。

## アクション

### [`install`](./install)

**メインアクション。** vendor キャッシュを復元し、ミス時は `composer install` を実行してキャッシュを保存します。
...
| output         | 説明                                                            |
|----------------|---------------------------------------------------------------|
| `cache-hit`    | キャッシュから `vendor/` を復元できたか（`true` なら `composer install` をスキップ） |
| `vendor-key`   | `vendor/` の保存・復元に使ったキャッシュキー                                   |
| `composer-key` | Composer ダウンロードキャッシュのキー                                        |

---

### [`vendor-build`](./vendor-build)（非推奨エイリアス）

`install` の下位互換エイリアスです。`install` に移行してください。

#### inputs / outputs

`install` と同一（ただし `cache-hit` output は公開しません）。

---

### [`vendor-load`](./vendor-load)

テスト・静的解析ジョブ用。`install` が保存した `vendor` キャッシュを復元します。
`actions/cache/restore@v5` の薄いラッパーで、inputs / outputs は `actions/cache/restore` と同一です。
`key` には `install`（または `vendor-build`）の `vendor-key` 出力を渡します。
...
| `key`                  | yes      | —       | 復元に使う明示的なキー（`install` の `vendor-key` を渡す）      |
...
jobs:
  test:
    ...
    steps:
      ...
      - uses: lepusinc/gha-composer/install@v1
        with: { working-directory: src }
      - run: composer run test
        working-directory: src
```

### 複数ジョブ（`install` + `vendor-load`）

build ジョブで `install` を実行し、後続の test / lint ジョブで `vendor-load` を使います。
`install` の `vendor-key` 出力をジョブ出力経由で `vendor-load` の `key` に渡すことで、
`vendor-load` は `composer.lock` に依存せず、build が保存したキャッシュをキー一致で復元できます。
